...
export class ProductsService {
  constructor(
    ...
    @InjectRepository(Brand) private brandRepo: Repository<Brand>,  // ← THÊM DÒNG NÀY
  ) {}
  ...
  async findAllProducts(
    ...
    // ✅ FILTER CATEGORY
    if (category) {
      query.andWhere('(category.slug = :category OR category.id = :category)', { category });
    }

    // ✅ FILTER BRAND - BỎ brand.slug
    if (brand) {
      query.andWhere('(brand.name = :brand OR brand.id = :brand)', { brand });
    }
  ...
  async findOneProduct(identifier: string): Promise<Product> {
  const product = await this.productRepo.findOne({
    where: [
      { id: identifier },      // ✅ Tìm theo id (UUID)
      { slug: identifier },    // ✅ HOẶC theo slug
    ],
    relations: ['category', 'brand', 'details', 'details.color', 'details.size', 'details.images'],
  });
  ...
  async searchProducts(keyword: string): Promise<Product[]> {
    return this.productRepo.find({ 
      ...
      relations: ['category', 'brand']  // ← THÊM 'brand'
    });
  }
  ...
  async createBrand(name: string, origin: string, brandCode: string): Promise<Brand> {
    const existing = await this.brandRepo.findOne({ where: { brandCode } });
    if (existing) throw new ConflictException('Brand code already exists');
    const brand = this.brandRepo.create({ name, origin, brandCode });
  ...
  async deleteBrand(id: number): Promise<void> {
    ...
    const productCount = await this.productRepo.count({ where: { brandId: id } });
    if (productCount > 0) {
      throw new ConflictException('Cannot delete brand because it is used by products');
    }
    await this.brandRepo.delete(id);
  }

  async searchBrands(keyword: string): Promise<Brand[]> {
    return this.brandRepo.find({
      where: [
        { name: Like(`%${keyword}%`) },
  ...
  async createColor(name: string, code?: string, hexCode?: string) { 
    return this.colorRepo.save({ name, code, hexCode }); 
  }
  ...
  async getBestSellers(limit: number = 8) {
    return this.productRepo.find({
      ...
      take: limit,
      relations: ['category', 'brand', 'details', 'details.images'],
    });
  }
