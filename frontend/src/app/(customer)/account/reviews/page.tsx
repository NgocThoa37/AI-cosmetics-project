...
export default function MyReviewsPage() {
  ...
  useEffect(() => {
    ...
    console.log('🔍 [Reviews] Token:', token ? 'CÓ' : 'KHÔNG');
    console.log('🔍 [Reviews] isAuthenticated:', isAuthenticated);
                  ...
                  <div key={review.id} className="bg-white border border-brand-warm rounded-2xl p-5 hover:shadow-md transition-shadow">
                    <div className="flex gap-4">
                      <Link href={`/products/${review.productId}`} className="w-20 h-20 rounded-lg overflow-hidden bg-brand-sand flex-shrink-0">
                        <Image
                          src={review.product?.details?.[0]?.images?.find(img => img.isMain)?.imageUrl || '/placeholder.jpg'}
                  ...
                  <div key={item.id} className="bg-white border border-brand-warm rounded-2xl p-5 hover:shadow-md transition-shadow">
                    <div className="flex gap-4">
                      <Link href={`/products/${item.productId}`} className="w-20 h-20 rounded-lg overflow-hidden bg-brand-sand flex-shrink-0">
                        <Image
                          src={item.product?.details?.[0]?.images?.find(img => img.isMain)?.imageUrl || '/placeholder.jpg'}
        ...
        <form onSubmit={handleSubmit} className="p-5 space-y-5 overflow-y-auto max-h-[70vh]">
          ...
          <div>
            ...
            <div className="flex gap-3 flex-wrap">
              ...
              {/* ✅ Nút chọn ảnh từ máy */}
              <label className="w-20 h-20 border-2 border-dashed border-brand-warm rounded-lg flex flex-col items-center justify-center text-brand-dark/40 hover:border-brand-accent hover:text-brand-accent transition-colors cursor-pointer">
                <input
                  type="file"
                  accept="image/*"
                  multiple
            ...
            {images.length > 0 && (
              <p className="text-xs text-brand-dark/40 mt-2">Đã chọn {images.length} ảnh</p>
            )}
          </div>
