...
import { useCustomerAuth } from '@/hooks/useCustomerAuth'; // ✅ Đổi sang useCustomerAuth
...
export default function CustomerLoginPage() {
  ...
  const { login, loading, error } = useCustomerAuth(); // ✅ Dùng customer auth
    ...
    <div className="min-h-screen flex items-center justify-center p-4">
      <div className="max-w-md w-full bg-white rounded-2xl shadow-xl p-8">
        <div className="text-center mb-8">
          ...
          <p className="text-sm text-brand-dark/50 mt-1">Khách hàng</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div className="flex justify-between items-center">
            <label className="flex items-center gap-2">
              <input type="checkbox" className="rounded border-brand-warm text-brand-accent" />
              <span className="text-xs text-brand-dark/60">Ghi nhớ</span>
            </label>
